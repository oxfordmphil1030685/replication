...
** iterations	
	...
	forval S = 6/14 { 	
		preserve
			...
			* Estimate model
			melogit $y D1 M1 D2 M2 D_* M_* $tc t  || id:, pw(W) 
			mat a = r(table)' 
	...
	* append estimates from org paper 
	...
	graph export "$dir/fig/snsvty_addwaves_drpt_bet_logit.eps", replace 
	...
	gr tw 	rcap ll ul Wave if var == 2 & org == 1, lcol(blue)		///
		...
		ylab(-.8(.4)2, angle(h)) yline(0, lpat(dash)) 				///
		legend(off)
	graph export "$dir/fig/snsvty_addwaves_drpt_wit_logit.eps", replace 
	...
	gr tw 	rcap ll ul Wave if var == 3 & org == 1, lcol(blue)		///
		...
		legend(off)
	graph export "$dir/fig/snsvty_addwaves_yrs_bet_logit.eps", replace 
	...
	gr tw 	rcap ll ul Wave if var == 4 & org == 1, lcol(blue)		///
		...
		ylab(-.6(.2).4, angle(h)) yline(0, lpat(dash)) 				///
		legend(off)
	graph export "$dir/fig/snsvty_addwaves_yrs_wit_logit.eps", replace 	
